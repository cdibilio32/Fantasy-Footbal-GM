...
import { enhancedCostMonitor } from './enhancedCostMonitor.js';
...
class ABTestingService {
  ...
  async executeVariant(
    ...
    if (variantConfig.useLLM) {
      ...
      // Track cost
      const costRecord = {
        provider: this.getProviderFromModel(variantConfig.llmModel!),
        model: variantConfig.llmModel!,
        operation,
      ...
      await enhancedCostMonitor.recordCost(costRecord);
