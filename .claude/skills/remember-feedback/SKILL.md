...
description: Capture feedback about the Fantasy Football AI Manager's recommendations (trade, waiver, or lineup output) into fantasy-engine/agents/memories.md so future agent runs apply it. Trigger on phrases like "remember this", "add that to memory", "update memory/memories", "learn from that/this feedback", or when the user corrects, praises, or states a standing preference about a recommendation trade_agent.py/waiver_agent.py/lineup_agent.py just produced.
---

# Remember Feedback

Turns a piece of user feedback into a durable, distilled entry in
`fantasy-engine/agents/memories.md`. That file is read on every agent run
(`common.py::load_memories()`) and appended to the agent's system prompt as a
"LEARNED PREFERENCES" section — so an entry written here changes behavior on
every future `trade_agent.py` / `waiver_agent.py` / `lineup_agent.py` run, not
just in chat.
...
- Implicit feedback on a recommendation an agent just produced or that's being
...
1. **Find the feedback.** Use the current turn if it's the feedback itself.
   ...
   look for the agent output/recommendation being discussed and what the user
...
3. **Read `fantasy-engine/agents/memories.md`.** Create it from this
...
5. **Commit the change.** Stage and commit just `fantasy-engine/agents/memories.md`
   ...
   Tell the user the entry is committed locally and ask if they want it pushed
   now (or note that it'll go out with their next normal push) — don't push on
   their behalf.
